 ...
 */
class TokenCollection extends \Illuminate\Database\Eloquent\Collection
{
     ...
     * Unfortunately, we can't store $recent_event_times directly in the collection,
     * because it won't get copied in other operations.
     ...
     * @param string $field_name optional The attribute name to use for this event in the User model.
     *                           If not specified, defaults to last_{$type}_time.
     *
    ...
    public function getRecentEvents($type, $field_name = null)
    {
        ...
        $recentEventsQuery = TokenEvent::mostRecentEventsByType($type);
        ...
        foreach ($recent_events as $event) {
            $recent_event_times[$event['token_id']] = $event['occurred_at'];
        }

        // Merge in recent event times, and set any missing values
        foreach ($this as $token) {
            if (isset($recent_event_times[$token->id])) {
                $token->{$field_name} = $recent_event_times[$token->id];
            } else {
                $token->{$field_name} = 0;   // Should be an integer, so it can be properly parsed on client-side
                $recent_event_times[$token->id] = 0;
            }
        }
